...
subject to vdef {i in buses}: v[i] = e[i] ^2 + f[i] ^2;

### ek em + fk fm
subject to cdef {i in branches}: c[i] = e[bus_f[i]] * e[bus_t[i]] + f[bus_f[i]] * f[bus_t[i]];

### -ek fm + fk em
subject to sdef {i in branches}: s[i] = -e[bus_f[i]] * f[bus_t[i]] + f[bus_f[i]] * e[bus_t[i]];
