...
param num_jabr_cuts >= 0;
#param drop_jabr_cuts >= 0;
...
set bus_Gs;
set bus_Bs;
set jabr_cuts dimen 2;

set drop_jabrs dimen 2;
...
param c_jabr {1..num_jabr_cuts};
param s_jabr {1..num_jabr_cuts};
param vf_jabr {1..num_jabr_cuts};
param vt_jabr {1..num_jabr_cuts};
...
var v {i in buses} >= Vmin[i] ^2, <= Vmax[i] ^2;
...
#OBJECTIVE                                                  
#minimize total_cost: (if obbt_min = 0 then sum {i in gens} (fixedcost[i] + lincost[i] * Pg[i] + quadcost[i] * Pg[i] ^2) else obbt_min * v[obbt_bus]);
...
#Loss inequalities                                                                                                       
#subject to ploss_ineq {i in branches}: Pf[i] + Pt[i] >= 0;

#subject to cs_linear {i in branches}: 2 * c[i] <= v[bus_f[i]] + v[bus_t[i]];

#subject to linear_cuts_f {i in mincut_cuts}: 2 * Pf[i] <= v[bus_f[i]] + I[i];

#subject to linear_cuts_t {i in mincut_cuts}: 2 * Pt[i] <= v[bus_t[i]] + I[i];

subject to jabr {i in branches}: c[i] ^2 + s[i] ^2 <= v[bus_f[i]] * v[bus_t[i]];

#subject to i2 {i in branches}: Pf[i] ^2 + Qf[i] ^2 <= v[bus_f[i]] * I[i];

#subject to j_cuts {(i,j) in jabr_cuts}:
#    c_jabr[i] * c[j] + s_jabr[i] * s[j] + vf_jabr[i] * v[bus_f[j]] + vt_jabr[i] * v[bus_t[j]] <= 0;
...
#QLoss inequalities
#subject to qloss_ineq {i in qloss}: Qf[i] + Qt[i] >= 0;
