...
set i2_cuts dimen 2;
set limf_cuts dimen 2;
set limt_cuts dimen 2;
...
#jabrcuts
param c_jabr {1..num_jabr};
param s_jabr {1..num_jabr};
param vf_jabr {1..num_jabr};
param vt_jabr {1..num_jabr};

#i2cuts
param Pf_i2 {1..num_i2};
param Qf_i2 {1..num_i2};
param vf_i2 {1..num_i2};
param i2f_i2 {1..num_i2};

#limcuts
param Pf_lim {1..num_limf};
param Qf_lim {1..num_limf};
param Pt_lim {1..num_limt};
param Qt_lim {1..num_limt};
...
subject to jabrcuts {(i,j) in jabr_cuts}:
    c_jabr[i] * c[j] + s_jabr[i] * s[j] + vf_jabr[i] * v[bus_f[j]] + vt_jabr[i] * v[bus_t[j]] <= 0;

subject to i2cuts {(i,j) in i2_cuts}:
    Pf_i2[i] * Pf[j] + Qf_i2[i] * Qf[j] + vf_i2[i] * v[bus_f[j]] + i2f_i2[i] * i2f[j] <= 0;

subject to limfcuts {(i,j) in limf_cuts}:
    Pf_lim[i] * Pf[j] + Qf_lim[i] * Qf[j] <= 1;

subject to limtcuts {(i,j) in limt_cuts}:
    Pt_lim[i] * Pt[j] + Qt_lim[i] * Qt[j] <= 1;
