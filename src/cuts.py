...
def loss_cuts(log,all_data):
    ...
    for branch in most_violated.keys():
        ...
        lossexp = LinExpr()
        constrname = "loss_ineq_"+str(branchcount)+"_"+str(f)+","+str(t)
...
def drop_loss(log,all_data):
    ...
    for branch in loss_cuts.keys():
        ...
        log.joint(' slack of the loss cut ' + str(branchcount) + "_" + str(f) + "," + str(t) + ' = ' + str(slack) + '\n')
        if ( slack < - threshold ) and (cut_age > cut_age_limit):
            log.joint(' -------\n')
            log.joint(' slack loss cut ' + str(branchcount) + "_" + str(f) + "," + str(t) + ' = ' + str(slack) + '\n')
...
def i2_cuts(log,all_data):
    ...
    for branch in most_violated.keys():
        ...
        if all_data['i2_validity']:
            ...
            violation    = coeff_Pft * mp_Pf + coeff_Qft * mp_Qf + coeff_cff * mp_cbusf + coeff_i2ft * mp_i2f
        ...
        cutexp     = LinExpr()
        constrname = "i2_cut_"+str(cutid)+"_"+str(branch.count)+"r_"+str(rnd)+"_"+str(f)+","+str(t)
...
def drop_i2(log,all_data):
    ...
    for key in all_data['i2_cuts'].keys():
        ...
        constrname    = "i2_cut_"+str(cutid)+"_"+str(branchid)+"r_"+str(cut_rnd)+"_"+str(f)+","+str(t)
        constr        = themodel.getConstrByName(constrname)
        slack         = - constr.getAttr("slack")
...
def limit_cuts(log,all_data):
    ...
    for branch in most_violated.keys():
        ...
        if all_data['limit_validity']:
            ...
            if from_or_to == 'f':
                mp_Pval                 = all_data['mp_Pfvalues'][branch]
                mp_Qval                 = all_data['mp_Qfvalues'][branch]
            elif from_or_to == 't':
                mp_Pval                 = all_data['mp_Ptvalues'][branch]
                mp_Qval                 = all_data['mp_Qtvalues'][branch]

            slack    = coeff_P * mp_Pval + coeff_Q * mp_Qval - z

            if slack > FeasibilityTol:
                log.joint(' this cut is not valid!\n')
                log.joint(' violation ' + str(slack) + '\n')
                log.joint(' values (a primal bound)' + ' P ' + str(mp_Pval) + ' Q ' + str(mp_Qval) + ' branch limit ' + str(u) + '\n')
                breakexit('check!')
            else:
...
def jabr_cuts(log,all_data):
    ...
    for branch in most_violated.keys():
        ...
        if all_data['jabr_validity']:
            ...
            mp_cbusf = all_data['mp_cvalues'][buses[count_of_f]]
            mp_cbust = all_data['mp_cvalues'][buses[count_of_t]]
            slack    = coeff_cft * mp_c + coeff_sft * mp_s + coeff_cff * mp_cbusf + coeff_ctt * mp_cbust

            if slack > FeasibilityTol:
                log.joint(' this cut is not valid!\n')
                log.joint(' violation ' + str(slack) + '\n')
                log.joint(' values (a primal bound)' + ' cft ' + str(mp_c) + ' sft ' + str(mp_s) + ' cff ' + str(mp_busf) + ' ctt ' + str(mp_cbust) + '\n' )
                breakexit('check!')
            else:
                log.joint(' valid cut at branch ' + str(branch.count) + ' with slack ' + str(slack) + '\n')
        ...
        cutexp = LinExpr()
        constrname = "jabr_cut_"+str(cutid)+"_"+str(branch.count)+"r_"+str(rnd)+"_"+str(f)+","+str(t)
...
def drop_jabr(log,all_data):
    ...
    for key in all_data['jabr_cuts'].keys():
        ...
        constrname    = "jabr_cut_"+str(cutid)+"_"+str(branchid)+"r_"+str(cut_rnd)+"_"+str(f)+","+str(t)
        constr        = themodel.getConstrByName(constrname)
        slack         = - constr.getAttr("slack")
    ...
    while linenum < numlines: 
        ...
        if thisline[0] == '#i2-envelope' and jabr:
            ...
            if all_data['jabr_validity']:
                ...
                mp_cbusf       = all_data['mp_cvalues'][buses[count_of_f]]
                mp_cbust       = all_data['mp_cvalues'][buses[count_of_t]]
                mp_violation   = coeff_cft * mp_c + coeff_sft * mp_s + coeff_cff * mp_cbusf + coeff_ctt * mp_cbust
                mp_relviolation   = mp_violation / ( ( coeff_cft**2 + coeff_sft**2 + coeff_cff**2 + coeff_ctt**2 )**0.5 ) 

                if mp_relviolation > FeasibilityTol:
                    ...
                    log.joint(' violation ' + str(mp_violation) + '\n')
                    log.joint(' relative violation ' + str(mp_relviolation) + '\n')
                    log.joint(' values (AC solution)' + ' cft ' + str(mp_c) + ' sft ' + str(mp_s) + ' cff ' + str(mp_busf) + ' ctt ' + str(mp_cbust) + '\n' )
                    breakexit('check!')
                else:
                    log.joint(' AC solution satisfies Jabr inequality at branch ' + str(branch.count) + ' with slack ' + str(mp_relviolation) + '\n')


            cutexp = LinExpr()
            constrname = "jabr_cut_"+str(cutid)+"_"+str(branchid)+"r_"+str(rnd)+"_"+str(f)+","+str(t)
            cutexp += coeff_cft * cvar[branch] + coeff_sft * svar[branch] + coeff_cff * cvar[buses[count_of_f]] + coeff_ctt * cvar[buses[count_of_t]]
            ...
            if all_data['i2_validity']:
                ...
                mp_cbust        = all_data['mp_cvalues'][buses[count_of_t]]
                mp_i2f          = computei2value(log,all_data,branch,mp_c,mp_s,mp_cbusf,mp_cbust)
                mp_violation    = coeff_Pft * mp_Pf + coeff_Qft * mp_Qf + coeff_cff * mp_cbusf + coeff_i2ft * mp_i2f
                mp_relviolation = mp_violation / ( ( coeff_Pft**2 + coeff_Qft**2 + coeff_cff**2 + coeff_i2ft**2 )**0.5 ) 
                ...
                if mp_relviolation > FeasibilityTol:
                    ...
                    log.joint(' AC solution satisfies i2 inequality at branch ' + str(branch.count) + ' with slack ' + str(mp_relviolation) + '\n')
            ...
            if all_data['limit_validity']:
                if from_or_to == 'f':
                    mp_P           = all_data['mp_Pfvalues'][branch]
                    mp_Q           = all_data['mp_Qfvalues'][branch]
                elif from_or_to == 't':
                    mp_P           = all_data['mp_Ptvalues'][branch]
                    mp_Q           = all_data['mp_Qtvalues'][branch]
                mp_violation    = coeff_P * mp_P + coeff_Q * mp_Q - 1

                if mp_violation > FeasibilityTol:
                    ...
                    log.joint(' violation ' + str(mp_violation) + '\n')
                    if from_or_to == 'f':
                        log.joint(' values (AC solution) ' + ' Pft ' + str(mp_P) + ' Qft ' + str(mp_Q) + '\n')
                    elif from_or_to == 't':
                        log.joint(' values (AC solution) ' + ' Ptf ' + str(mp_P) + ' Qtf ' + str(mp_Q) + '\n')
                    breakexit('check!')
                else:
                    log.joint(' AC solution satisfies limit inequality at branch ' + str(branch.count) + ' with slack ' + str(mp_violation) + '\n')


            cutexp     = LinExpr()
            if from_or_to == 'f':
                constrname = "limit_cut_"+str(cutid)+"_"+str(branch.count)+"r_"+str(rnd)+"_"+str(f)+","+str(t)
                cutexp    += coeff_P * Pvar_f[branch] + coeff_Q * Qvar_f[branch]
            elif from_or_to == 't':
                constrname = "limit_cut_"+str(cutid)+"_"+str(branch.count)+"r_"+str(rnd)+"_"+str(f)+","+str(t)
                cutexp    += coeff_P * Pvar_t[branch] + coeff_Q * Qvar_t[branch]
