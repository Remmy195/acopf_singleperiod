...
def goac_ef(log,all_data):
    ...
    e          = ampl.get_variable("e")
    f          = ampl.get_variable("f")    
    ...
    dic_e      = e.get_values().to_dict()
    dic_f      = f.get_values().to_dict()
    ...
    filename      = 'ksol_ef_' + all_data['casename'] + '.txt'
    filenamelp    = 'ksol_ef_' + all_data['casename'] + '.lp'
    ...
    thefile.write('voltages and efs:\n')

    for buscount in buses.keys():
        vval       = dic_v[buscount]
        e_val      = dic_e[buscount]
        f_val      = dic_f[buscount]
        f          = buses_data[buscount].nodeID
        line = 'bus ' + str(buscount) + ' M ' + str(vval) + ' e ' + str(e_val) + ' f ' + str(f_val) + '\n'
        thefile.write(line)
    ...
    for branchid in branches.keys():
        ...
        line = 'branch ' + str(branchid) + ' f ' + str(f) + ' t ' + str(t) + ' Pft ' + str(Pfval) + ' Ptf ' + str(Ptval) + ' Qft ' + str(Qfval) + ' Qtf ' + str(Qtval) + '\n'
    ...
    if writedowngen:
        ...
        for genid in gens.keys():
            ...
            line_Pg = 'genid ' + str(genid) + ' bus ' + str(gen_nodeID) + ' GP ' + str(dic_Pg[genid]) + '\n'
            thefile.write(line_Pg)
