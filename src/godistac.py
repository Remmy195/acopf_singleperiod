...
def godistac(log,all_data):
    ...
    if all_data['solver'] == 'gurobi_ampl':
        ampl.eval("options option gurobi_options 'method=2 bar=1';")
    ...
    getsolrelax2(log,all_data) # getting solution from relaxation
    v0     = all_data['v0']
    theta0 = all_data['theta0']
    ...
    ampl.get_parameter('v0').set_values(v0) # from cutplane solution
    ampl.get_parameter('theta0').set_values(theta0) # from cutplane solution
    ...
    #GET SOLUTION
    distance   = ampl.get_objective("distance")
    val        = ampl.get_variable("val")
    ...
    objval    = distance.get().value()
    timesofar = time.time() - all_data['T0']

    log.joint(" value " + str(val.get().value()) + '\n')
    ...
    thefile.write('value ' + str(distance.get().value()) + '\n')
...
def getsolrelax(log,all_data):
    
    casename = all_data['casename']
    filename = 'cutplane_sols/cutplanesol_'+ casename +'.txt'
    
    log.joint(" reading file with relaxed solution " + filename + "\n")
    ...
    while linenum < lenlines:
        thisline = lines[linenum].split(' ')
        varname  = thisline[0].split('_')

        #log.joint(' thisline ' + str(thisline) + '\n')
        #log.joint(' varname ' + str(varname) + '\n')
        #log.joint(' len ' + str(len(varname)) + '\n')
        if len(varname) == 3 and varname[0] == 'c':
            bus_id       = int(varname[1])
            buscount     = IDtoCountmap[bus_id]
            v0[buscount] = math.sqrt(float(thisline[2]))
        elif varname[0] == 'c':
            branchid     = int(varname[1])
            c0[branchid] = float(thisline[2])
            log.joint(' branchid ' + str(branchid) + ' c0 ' + '\n')
        elif varname[0] == 's':
            branchid     = int(varname[1])
            s0[branchid] = float(thisline[2])
            log.joint(' branchid ' + str(branchid) + ' s0 ' + '\n')
        elif varname[0] == 'GP':
            gencount      = int(varname[1])
            genid         = int(varname[2])
            Pg0[gencount] = float(thisline[2])
        elif varname[0] == 'GQ':
            gencount      = int(varname[1])
            genid         = int(varname[2])
            Qg0[gencount] = float(thisline[2])
    ...
    breakexit('c')
    for branchid in c0.keys():
        f = branches[branchid].f
        t = branches[branchid].t
        theta0[branchid] = math.atan2(s0[branchid],c0[branchid])
        log.joint( ' branchid ' + str(branchid) + ' f ' + str(f) + ' t ' + str(t) + '\n')
        log.joint(' s0 ' + str(s0[branchid]) + ' c0 ' + str(c0[branchid]) + ' theta0 ' + str(theta0[branchid]) + '\n')
    
    all_data['v0']     = v0
    all_data['theta0'] = theta0


def getsolrelax2(log,all_data):
    ...
    while linenum < lenlines:
        ...
        if len(varname) == 3 and varname[0] == 'c':
            ...
            buscount     = IDtoCountmap[bus_id]
            v0[buscount] = math.sqrt(float(thisline[2]))
            log.joint(' v0 ' + str(v0[buscount]) + '\n')
        elif varname[0] == 'theta':
            buscount         = int(varname[1])
            theta0[buscount] = float(thisline[2])
            log.joint(' theta0 ' + str(theta0[buscount]) + '\n')
        elif varname[0] == 'GP':
            gencount      = int(varname[1])
            genid         = int(varname[2])
            Pg0[gencount] = float(thisline[2])
