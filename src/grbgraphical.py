...
def grbgraphical(alldata, plottype, textlist):
    ...
    for branch in branches.values():
        graph_dict[counter] = (branch.f, branch.t)
        counter += 1
    ...
    for j in range(1, numbuses + 1):
        bus = buses[j]
        bus_id = bus.nodeID
        mynode_size[bus_id] = 1
        mynode_color[bus_id] = "black"
        mynode_border_width[bus_id] = 1
    ...
    if plottype == "violation":
        ...
        for j in range(1, numbuses + 1):
            bus = buses[j]
            bus_id = bus.nodeID
            node_text[bus_id] = "Bus %d Vmagviol: %.3e Pviol %.3e Qviol %.3e" % (
                j,
                Vmagviol[bus],
                IPviol[bus],
                IQviol[bus],
            ...
            if (
                ...
                mynode_size[bus_id] = 15
                mynode_color[bus_id] = "red"
        ...
        for j in range(1, numbuses + 1):
            ...
            sumPgen_MW = sumPgen * 100

            largegen, mynode_size[bus_id], mynode_color[bus_id] = grbgetgraphattr(
                alldata, sumPgen_MW
            )

            Pload = alldata["baseMVA"] * bus.Pd
            node_text[bus_id] = "Bus %d   Gen %7.2f  Load %7.2f" % (
                bus.nodeID,
                sumPgen,
                Pload,
            )
            ...
            if Pload >= 50 and largegen is False:
                mynode_size[bus_id] = 7
                mynode_color[bus_id] = "blue"
    ...
    for j in range(1, numbranches + 1):
        ...
        myedge_ends[(branch.f, branch.t)] = j
        myedge_ends[(branch.t, branch.f)] = j
        small = min(branch.f, branch.t)
        large = max(branch.f, branch.t)
...
def graphplot(
    ...
    n = graph_dict["N"]
    m = graph_dict["M"]
    print(len(graph_dict), n, m)
    # Map: bus_id -> 0..n-1 index used by vertexx/vertexy
    IDtoCount = alldata["IDtoCountmap"]

    adj = {}
    raw_nodes = set()
    for i in range(m):
        u = int(graph_dict[i][0])   # e.g., 7351
        v = int(graph_dict[i][1])   # e.g., 5441
        adj[i] = [u, v]
        raw_nodes.update((u, v))
    ...
    # ... after vertexx/vertexy scaling
    pos = {}
    for bus_id, idx in IDtoCount.items():
        pos[bus_id] = [float(vertexx[idx-1]), float(vertexy[idx-1])]


    # Construct network graph
    graph_vertices = set(pos.keys())
    graph_edges = []
    ...
    for j in range(m):
        ...
        # Add this debugging block to find the invalid data
        if not (small in graph_vertices and large in graph_vertices):
            print(f"!!! ASSERTION FAILED at edge index {j} !!!")
            print(f"    Edge nodes: ({small}, {large})")
            print(f"    Total number of nodes (n): {n}")
            print(f"    Valid node range is 0 to {n-1}.")
            print(f"    Original edge data from graph_dict[{j}]: {graph_dict[j]}")
            raise AssertionError(f"Edge {j} connects to an invalid node.")
