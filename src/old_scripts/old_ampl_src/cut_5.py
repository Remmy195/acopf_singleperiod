...
def cutheuristic_5(log,all_data):
    ...
    buses = copy.deepcopy(all_data['goampl_buses'])
    branches = copy.deepcopy(all_data['goampl_branches'])
    bus_f = copy.deepcopy(all_data['goampl_bus_f'])
    bus_t = copy.deepcopy(all_data['goampl_bus_t'])
    branches_f = copy.deepcopy(all_data['goampl_branches_f'])
    branches_t = copy.deepcopy(all_data['goampl_branches_t'])
    zero_r = copy.deepcopy(all_data['goampl_zero_r'])
    PLoss = copy.deepcopy(all_data['PLoss'])
    s = copy.deepcopy(all_data['source'])
    t = copy.deepcopy(all_data['target'])
    ...
    branches_t_set[super_source].setValues(branches_t[super_source]) 
    ...
    for constr in constrs_f:
        if constr[1].dual() > 1e-6:
            ...
            if constrID not in all_data['mincut_cuts'] and (constrID not in delta_s and constrID not in delta_t):
                all_data['new_mincut'] = 1
                all_data['mincut_cuts'].append(int(constrID))
                added = 1
                counter_mincut += 1
    ...
    all_data['num_mincut_cuts'] += counter_mincut

    print("current set of cuts (by branchids) =",all_data['mincut_cuts'])
    print("current number of cuts (min-cut heuristic) =",all_data['num_mincut_cuts'])
    print("current number of cuts (min-cut heuristic) =",len(all_data['mincut_cuts']))
