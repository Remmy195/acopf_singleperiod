...
def cutjabr_5(log,all_data):
    ...
    for i in branches:
        ...
        if violation[i] > 1e-05:
            ...
            print("LHS: cft = {0} , sft = {1}, vff - vtt = {2}".format(2 * c[i],2 * s[i],v[bus_f[i]] - v[bus_t[i]]))
            print("cutnorm = {0}\n".format(math.sqrt( (2 * c[i])**2 + (2 * s[i])**2 + (v[bus_f[i]] - v[bus_t[i]])**2)))
            print("-----\n")
