...
import time
from cut_2 import cutheuristic_2

import igraph as ig
import matplotlib.pyplot as plt


def goampl_2(log,all_data):
    ...
    print("set_t",set_t)
    #print(set_s.keys())
    breakexit("check dictionaries")

    #s = max(netgen, key = netgen.get)
    #t = min(netgen, key = netgen.get)

    print("source = {0}, target = {1}".format(set_s,set_t))
    ...
    print("dicLoss",dicLoss)
    all_data['goampl_source'] = list(set_s.keys()) #before s
    all_data['goampl_target'] = list(set_t.keys()) #before t
