    ...
    while iteration <= max_iterations:
        ...
        for loss in dicLoss:
            if dicLoss[loss] <= 1e-06:
                dicLoss[loss] = 0
        dicQLoss = {}
        for branch in dicQf.keys():
            if math.fabs(dicQf[branch] + dicQt[branch]) <= 1e-06:
                dicQLoss[branch] = 0 
            else:
                dicQLoss[branch] = dicQf[branch] + dicQt[branch]
            print("branch = {0} Qf + Qt = {1}".format(branch,dicQLoss[branch]))
        ...
        print(" current number of min-cut cuts",len(all_data['cuts']))
