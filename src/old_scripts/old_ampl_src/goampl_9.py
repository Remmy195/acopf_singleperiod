...
from cutjabr_9 import cutjabr_9
from obbt_7 import obbt_7
#import igraph as ig
#import matplotlib.pyplot as plt


def goampl_9(log,all_data):
    ampl = AMPL()
    ...
    #drop CUTS initialized empty
    all_data['drop_jabrs'] = []
    ampl.getSet('drop_jabrs').setValues(all_data['drop_jabrs'])
    ...
    while all_data['round'] <= MAX_ROUNDS:
        ...
        print(" current number of jabr cuts",len(all_data['jabr_cuts']))
        ...
        #cutjabr_analysis_9(log,all_data)
        ...
        for key in all_data['jabr_cuts']:
            ...
            elif rnd <= all_data['round'] - 20:
                drop_jabrs.append(key)
                print("the cut = {0} was added to drop_jabrs list".format(cut))
                
            print(" ------------\n")

        print(" num simple jabr-cuts per rnd",num_jabr_cuts_rnd)
        print(" num tight simple jabr-cuts",tight_cuts)
        print(" % of tight simple jabr-cuts",tight_cuts_fraction)

        #drop criteria: if slack more than original threshold and 20
        if len(drop_jabrs):
            all_data['drop_jabrs'].extend(drop_jabrs)


            for cut_id in drop_jabrs:
                print("cut_id",cut_id)
                all_data['total_jabrs'] -= 1
                all_data['jabr_cuts'].pop(cut_id,None)
                print("the cut = {0} was removed".format(cut_id))

            ampl.getSet('drop_jabrs').setValues(all_data['drop_jabrs'])     


        print(" round =",all_data['round'])
        print(" current objective = ",total_cost.get().value())
        print(" total number of jabr cuts",all_data['total_jabrs'])
        print(" max error in current round = {0} at branch = {2} branchid = {1}".format(all_data['max_error_rnd'],all_data['most_violated_branch_rnd'],branches[all_data\
['most_violated_branch_rnd']]))
        print(" number of added jabrs in current round",all_data['num_jabr_cuts_rnd'][all_data['round']])
        print(" number of dropped jabrs in current round",len(drop_jabrs))
        print(" current threshold",all_data['threshold'])
        print(" time so far:",time.time()-all_data['timestart'])
